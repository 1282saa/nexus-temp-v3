        ...
        if (session && session.tokens) {
          ...
          if (user) {
            ...
            if (user.email === import.meta.env.VITE_ADMIN_EMAIL) {
              userRole = 'admin';
              userPlan = 'premium';
              console.log('🔐 관리자 권한 부여 (${import.meta.env.VITE_ADMIN_EMAIL})');
            }
  ...
  const handleSubmit = async (onLoginSuccess) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Only ${import.meta.env.VITE_ADMIN_EMAIL} is admin
        if (formData.username === import.meta.env.VITE_ADMIN_EMAIL || result.user.email === import.meta.env.VITE_ADMIN_EMAIL) {
          userRole = 'admin';
          userPlan = 'premium';
        }
    ...
    try {
      ...
      if (result.success) {
        ...
        if (loginResult.success) {
          ...
          let userRole = 'user';
          // Only ${import.meta.env.VITE_ADMIN_EMAIL} is admin
          if (formData.username === import.meta.env.VITE_ADMIN_EMAIL || loginResult.user.email === import.meta.env.VITE_ADMIN_EMAIL) {
            userRole = 'admin';
          }
          localStorage.setItem('userRole', userRole);
