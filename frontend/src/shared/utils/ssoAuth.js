 ...
 * SSO 서버에서 전달된 JWT 토큰을 처리하여 자동 로그인을 수행합니다.
 */

import { ssoConfig, getCognitoIssuer, isAdminEmail } from '../../utils/ssoConfig';
...
export const getTokensFromURL = () => {
  ...
  // 방법 2: localStorage에서 읽기 (App.jsx에서 미리 저장한 것)
...
const validateToken = (idToken) => {
  try {
    ...
    // Cognito User Pool 확인
    const expectedIssuer = getCognitoIssuer();
...
export const attemptSSOLogin = async () => {
  try {
    ...
    if (isAdminEmail(userInfo.email)) {
      userRole = 'admin';
      userPlan = 'premium';
      console.log('🔐 관리자 권한 부여:', userInfo.email);
    } else {
    ...
    localStorage.setItem('refreshToken', accessToken); // SSO는 refreshToken이 없으므로 accessToken 사용
